        ...
        {projects.map((project) => (
          <li key={project.title} className="cursor-default">
            <div className="project-inner portfolio-card-shadow flex h-full flex-col items-start justify-between rounded-[var(--border-radius)] bg-[var(--light-navy)] p-7 transition-[var(--transition)] [@media(max-width:768px)]:p-6">
              <div>
                <p className="mb-2 font-mono text-[length:var(--fz-xxs)] text-[var(--green)]">
                ...
                {project.impact?.length ? (
                  <ul className="mt-4 flex list-none flex-wrap gap-2 p-0">
                    {project.impact.slice(0, 2).map((metric) => (
                      <li
                        key={metric}
                        className="rounded-full border border-[var(--green)] bg-[var(--green-tint)] px-2.5 py-1 font-mono text-[length:var(--fz-xxs)] text-[var(--green)]"
                      >
                        {metric}
                      </li>
                    ))}
